...
then
  ...
  do
    ...
    do
      ...
      echo ${instance}
	    timeout -k 5s 1m ./../../../build_release/expansionTests ${domainType} ${lookahead} ../../../results/PancakeDPS/expansionTests/Nancy/${dimensions}/LA${lookahead}-${instance}.json < ${file}
	    let instance++
      let numProcs++
	  fi
    done
  ...
  echo "Available domain types are Pancake,PancakeDPS, TreeWorld, and SlidingPuzzle"
  echo "Domain variables for TreeWorld: <branching factor> <tree depth>"
  echo "Domain variables for SlidingPuzzle: <puzzle dimensions>"
