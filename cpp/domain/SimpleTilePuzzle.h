...
public:
	...
	public:
		...
		friend std::ostream& operator<<(std::ostream& stream, const SimpleTilePuzzle::State& state) {
	...
	struct HashState
	{
		std::size_t operator()(const State &s) const
		{
			...
			for (int i = 7; i >= 0; i--)
			{
				...
				hash = hash ^ SimpleTilePuzzle::table[byte];
			}
			cout << key << " " << hash << endl;
			return hash;
			*/
	...
	SimpleTilePuzzle(std::istream& input) {
		...
		if (SimpleTilePuzzle::table.empty())
		{
			srand(time(NULL));
			for (int i = 0; i < 256; i++)
			{
...
vector<int> SimpleTilePuzzle::table;
