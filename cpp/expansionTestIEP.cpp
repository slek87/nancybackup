...
int main(int argc, char** argv){
	...
	if (domain == "Pancake" || domain == "PancakeDPS" ){
		...
		RealTimeSearch<PancakePuzzle> iep(world, "iep", "learn", "minimin", lookaheadDepth);
		res = iep.search();
		if (!world.validatePath(res.path)) exit(1);

	} else if (domain == "TreeWorld"){
		TreeWorld world = TreeWorld(cin);
		RealTimeSearch<TreeWorld> iep(world, "iep", "none", "minimin", lookaheadDepth);
		res = iep.search();

	} else if (domain == "SlidingPuzzle") {
		SlidingTilePuzzle world = SlidingTilePuzzle(cin);
		RealTimeSearch<SlidingTilePuzzle> iep(world, "iep", "learn", "minimin", lookaheadDepth);
		res = iep.search();
		if (!world.validatePath(res.path)) exit(1);

	} else if (domain == "HeavyTile") {
		HeavyTilePuzzle world = HeavyTilePuzzle(cin);
		RealTimeSearch<HeavyTilePuzzle> iep(world, "iep", "learn", "minimin", lookaheadDepth);
		res = iep.search();
		if (!world.validatePath(res.path)) exit(1);

	} else if (domain == "InverseTile") {
		InverseTilePuzzle world = InverseTilePuzzle(cin);
		RealTimeSearch<InverseTilePuzzle> iep(world, "iep", "learn", "minimin", lookaheadDepth);
		res = iep.search();
		if (!world.validatePath(res.path)) exit(1);
