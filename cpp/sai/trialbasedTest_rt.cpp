...
int main(int argc, char** argv) {
    ...
    if (domain == "Pancake" || domain == "PancakeDPS" ){
        ...
        THTS_RT <PancakePuzzle> thts(world, algorithm, lookahead);
        if (prune_type == "lock"){
        ...
        THTS_RT <TreeWorld> thts(world, algorithm, lookahead);
        if (prune_type == "lock"){
            thts.setPruning(prune_type);
        }
        res = thts.getPlan();
        ...
        THTS_RT <SlidingTilePuzzle> thts(world, algorithm, lookahead);
        if (prune_type == "lock"){
            thts.setPruning(prune_type);
        }
        res = thts.getPlan();
        ...
        THTS_RT <HeavyTilePuzzle> thts(world, algorithm, lookahead);
        if (prune_type == "lock"){
            thts.setPruning(prune_type);
        }
        res = thts.getPlan();
        ...
        THTS_RT <InverseTilePuzzle> thts(world, algorithm, lookahead);
        if (prune_type == "lock"){
            thts.setPruning(prune_type);
        }
        res = thts.getPlan();
