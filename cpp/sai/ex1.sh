...
if [ "$1" = "help" ] || [ "$1" = "-help" ] || [ "$1" = "?" ]
then
  echo "./ex1.sh <starting instance #> <# of instances to test> <# of processes> <Domain Type>"
  exit 1
fi
...
then
    dimensions=4
    mkdir -p results/ex1/${domainType}/${dimensions}x${dimensions}
    ...
    do
        ...
        if [ -f results/ex1/${domainType}/${dimensions}x${dimensions}/${instance}.json ]
        then 
        let instance++
        else
        ./ex1 ${domainType} results/ex1/${domainType}/${dimensions}x${dimensions}/${instance}.json < ${file} &
        let instance++
        let numProcs++
        fi
    done
...
then
    dimensions=10
    mkdir -p results/ex1/${domainType}/${dimensions}
    instance=$firstInstance
    while ((instance < lastInstance))
    do
      ...
      if [ -f results/ex1/${domainType}/${dimensions}/${instance}.json ]
	  then 
	    let instance++
	  else
      echo ${instance}
	    ./ex1 ${domainType} results/ex1/${domainType}/${dimensions}/${instance}.json < ${file}
	    let instance++
      let numProcs++
	  fi
    done
