...
public:
            ...
            if (it == TT.end()){
                ...
                if (trial_backup == "nancy"){
                     DiscreteDistribution d = DiscreteDistribution(100, childNode->getFValue(), childNode->getFHatValue(),
						    childNode->d, childNode->getFHatValue() - childNode->getFValue());
                    childNode->value = d.expectedCost();
                } else if (trial_backup == "nancy-greedy"){
                     DiscreteDistribution d = DiscreteDistribution(100, childNode->h, childNode->getHHatValue(),
						    childNode->d, childNode->getHHatValue() - childNode->h);
                    childNode->value = d.expectedCost();
                } else if (trial_backup == "bellman"){
                    childNode->value = childNode->getFHatValue();
    ...
    Node* selectNancy(Node* n, unordered_map<State, Node*, Hash>& TT){
        ...
        for (Node* child : n->successors){
            ...
            DiscreteDistribution d = DiscreteDistribution(100, best->getFValue(), best->getFHatValue(),
						    best->d, best->getFHatValue() - best->getFValue());
    ...
    void backUp(Node* n, unordered_map<State, Node*, Hash>& TT){
        ...
        if (trial_backup == "bfs" ){
            ...
            double value = 0;
            for (Node* child : n->successors){
                value += child->visits * (child->value + k * domain.getEdgeCost(child->state));
                visits += child->visits;
